...
//    private static final int VIEW_TYPE_PUB =2 ;
//    private static final int VIEW_TYPE_IMG =3 ;
...
//            adView = (AdView) view.findViewById(R.id.adView);
    ...
    @Override
    public void onBindViewHolder(final ForecastAdapterViewHolder forecastAdapterViewHolder, int position) {
         ...
         /*   case VIEW_TYPE_PUB:
                ...
                if(adRequest==null){
                    adRequest = new AdRequest.Builder()
                            .addTestDevice(mContext.getString(R.string.test_device))
                            .build();
                    forecastAdapterViewHolder.adView.loadAd(adRequest);
                    forecastAdapterViewHolder.adView.setAdListener(new AdListener() {
                        @Override
                        public void onAdLoaded() {
                            super.onAdLoaded();
                            forecastAdapterViewHolder.adView.setVisibility(View.VISIBLE);
                        }
                    });
                }
                break;*/
...
//        String description = Utility.getStringForWeatherCondition(mContext, weatherId);                                         //orig
        String description = mCursor.getString(ForecastFragment.COL_WEATHER_DESC);                                                                 // adaugat eu
...
//        return (position == 0 && mUseTodayLayout) ? VIEW_TYPE_TODAY : VIEW_TYPE_FUTURE_DAY;
        if (position == 0 && mUseTodayLayout) return VIEW_TYPE_TODAY;
//        else if (position == 3) return VIEW_TYPE_PUB;
